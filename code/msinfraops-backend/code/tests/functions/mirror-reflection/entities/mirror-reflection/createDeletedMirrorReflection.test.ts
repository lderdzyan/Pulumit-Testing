...
import { createDeletedMirrorReflection } from '@/lib/entities/mirror-reflection/mirror-reflection';
import { mrId, validMRItem } from '../../../../utils/mirror-reflection/data'; // Заменить на свой валидный mock
...
describe('createDeletedMirrorReflection', () => {
  ...
  it('calls callFunction with correct arguments and returns Right', async () => {
    ...
    const task = createDeletedMirrorReflection(validMRItem);
    const result = await task();

    expect(mockedCallFunction).toHaveBeenCalledWith(AWSMirrorReflection.createDeletedMirrorReflection);
    expect(innerFn).toHaveBeenCalledWith(expect.anything(), validMRItem);
  ...
  it('returns Left if callFunction fails', async () => {
    const error = new Error('DDB delete error');
